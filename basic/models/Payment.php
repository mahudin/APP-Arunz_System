...
class Payment
{
    ...
    public function generateToken($params){
        //https://direct.paylane.com/rest.js/cards/generateToken
    ...
    protected function call_token($method, $request, $params)
    {
        ...
        $response = $this->pushDataToken($method, $request, $params_encoded);
    ...
    protected function pushData($method, $method_type, $request)
    {
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER,0);// $this->ssl_verify);
    ...
    protected function pushDataToken($method, $method_type, $request)
    {
        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, 'https://direct.paylane.com/rest.js/' . $method);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $request);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-type: application/json'));
        curl_setopt($ch, CURLOPT_ENCODING, 'gzip,deflate');
